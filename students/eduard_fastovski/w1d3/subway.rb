...
#returns just the index number where they got on and where they got off 
...
# if the person is changing lines
  if @line_on != @line_off 
    
    @numstops = (unionin - index_on).abs + (unionout - index_off).abs   
    # abs turns negatives into positives
...
# THE FIRST HALF OF THE TRIP
    ...
    if index_on < unionin
      index_on.upto(unionin) { |stop| stops_covered.push(array_on[stop]) } #push each stop to the stops_covered array
    #If the index where they got on is higher than union square, list the stations down to union square
    elsif  index_on > unionin
      index_on.downto(unionin) { |stop| stops_covered.push(array_on[stop]) }
    end
# THE SECOND HALF OF THE TRIP
    if index_off > unionout
      unionout.upto(index_off) { |stop| stops_covered.push(array_off[stop]) }
    elsif  index_off < unionout
      unionout.downto(index_off) { |stop| stops_covered.push(array_off[stop]) }
    end
